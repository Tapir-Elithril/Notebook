...
a handler for each event type:interrupts(hardware) & exceptions(traps)(software) "processEvent()"
...
```
P0_f(){
P0_C2+1  cpu_switch_to(current,P1)
}
```
P0_C2+1
next_pc(P1)
...
user process -> call system call -> execute system call -> return 
...
#### Schedule of nonlinear pipeline(无大题)
...
##### synchronization examples(大题 <= bounded-buffer)
考研题练习
