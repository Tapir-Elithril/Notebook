...
def affine_backward(dout, cache):
    ...
    # begin answer
    # y = w * x + b, dout = ∂L / ∂y
    # dx = ∂L / ∂x = ∂L / ∂y * ∂y / ∂x = dout @ w.T
    dx = dout @ w.T # (N, D)
    dx = dx.reshape(x.shape) # (N, d_1, ..., d_k)
    # dw = ∂L / ∂w = ∂L / ∂y * ∂y / ∂w = x.T @ dout
    N = x.shape[0]
    x_reshaped = x.reshape(N, -1) # (N, D)
    dw = x_reshaped.T @ dout # (D, M)
    # db = ∂L / ∂b = ∂L / ∂y * ∂y / ∂b = dout * 1
...
def relu_backward(dout, cache):
    ...
    #############################################################################
    # begin answer
    # y = relu(x) = max(0, x), dout = ∂L / ∂y
    # dx = ∂L / ∂x = ∂L / ∂y * ∂y / ∂x = 0,if x < 0;dout,if x >= 0
    dx = np.where(x < 0, 0, dout)
    # end answer
    return dx


def svm_loss(x, y):# hinge loss
...
def softmax_loss(x, y):
    ...
    Returns:
      ...
      dx: Gradient of the loss with respect to x
    """
    # L_i = -\log(\frac{e^{x_{y_i}}}{\sum_{j=1}^C e^{x_j}})
