...
void Preorder(Tree T,int n,FILE *file)/*n is used for deleting the space at the end of the output*/
{
    if(T!=NULL)
    {
        if(cnt!=n)/*count whether we are going to print the last element of T1*/
        {
            fprintf(file,"%d ",T->Element);cnt++;
            Preorder(T->Left,n,file);
            Preorder(T->Right,n,file);
...
void Compare(int N,int X,Tree T,FILE *file)
{
    if(T!=NULL)
    {
        if(X+T->Element==N)
...
void Findbyinorder(Tree T1,Tree T2,int N,int maxT2,int minT2,int del,FILE *file)
{
    if(T1!=NULL)
    {
        while(T1->Left!=NULL&&T1->Left->Element==T1->Element){T1=T1->Left;}
        /*ensure the same equation only appears once when traversing T1*/
        Findbyinorder(T1->Left,T2,N,maxT2,minT2,del,file);
        ...
        Findbyinorder(T1->Right,T2,N,maxT2,minT2,del,file);
...
void FindNumber(int N,Tree T1,Tree T2,int del,FILE *file)
{
        int maxT2=Findmax(T2);
        int minT2=Findmin(T2);
        /*used for deserting unnecessary recursions*/
        Findbyinorder(T1,T2,N,maxT2,minT2,del,file);
    if(mark==0){fprintf(file,"false\n"); }
