    ...
    def _solve_support(self, row_support, col_support):
        ...
        for i in range(self.n_rows):
            if i in row_support:
                ...
                for j in range(self.n_cols):
                    if j in col_support:
                        u1_ij = self.M[i,j]  #u1(i,j) = i @ A @ j，其中i,j是纯策略，计算结果为self.M[i,j]
                        eq_row[self.n_rows + j] = u1_ij
                eq_row[self.n_rows + self.n_cols] = -1
                eq_constraints.append(eq_row)
                eq_rhs.append(0)
               # ∑_j u_1(i,j)*x_2*(j) - U_1 = 0
            ...
            if j in col_support:
                ...
                for i in range(self.n_rows):
                    if i in row_support:
                        u2_ij = -self.M[i,j] 
                        eq_row[i] = u2_ij
                eq_row[self.n_rows + self.n_cols + 1] = -1
                eq_constraints.append(eq_row)
                eq_rhs.append(0)
               # ∑_i u_2(i,j)*x_1*(i) - U_2 = 0
        ...
        for i in range(self.n_rows):
            if i not in row_support:
                ...
                for j in range(self.n_cols):
                    if j in col_support:
                        u1_ij = self.M[i,j]  
                        ineq_row[self.n_rows + j] = u1_ij
                ineq_row[self.n_rows + self.n_cols] = -1
            ...
            if j not in col_support:
                ...
                for i in range(self.n_rows):
                    if i in row_support:
                        u2_ij = -self.M[i,j]  
                        ineq_row[i] = u2_ij
                ineq_row[self.n_rows + self.n_cols + 1] = -1
                ineq_constraints.append(ineq_row)
                ineq_rhs.append(0)
               # ∑_i u_2(i,j)*x_1*(i) - U_2 ≤ 0
        #--------------------------------------------
